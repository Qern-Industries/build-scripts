...
set -x
cd "${BWDIR}" || exit
touch "${repolocation}"/log/"${date}"/6700k || true
touch "${repolocation}"/log/"${date}"/4700u || true
...
makepkg -s 2>&1 | tee -a "${repolocation}"/log/"${date}"/6700k || true
...
makepkg -s 2>&1 | tee -a "${repolocation}"/log/"${date}"/4700u || true
